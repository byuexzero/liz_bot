...
from liz_bot import daily_funcs, judge_detail, pending, replies, song_alias, song_query
...
async def handle_command(cmd_name, cmd_params, *, session_key: str | None = None):
    ...
    **指令名大小写不敏感**（``/HELP`` == ``/help``）：查表前统一转小写。
    ...
    if count < entry.min_params:
        return replies.text("router.bad_params", usage=usage)
    if entry.max_params is not None and count > entry.max_params:
        return replies.text("router.too_many_params", usage=usage)
    ...
    return _HANDLERS[entry.key](cmd_params, miss, session_key)
    ...
    * 解析不了 → **取消选择**，把这条消息当**新的关键词**重跑同一条指令。
    ...
    if index is None:
        ...
        return await handle_command(
            entry.names[0], message_content.split(), session_key=session_key
        )
...
async def _resume_pending(
    session_key: str | None, message_content: str
) -> str | None:
    ...
    if state.choices is not None:
        return await _resume_choice(session_key, entry, state, message_content)
    ...
    # 够了 —— **先消费掉状态再执行**。
    # 执行失败（比如参数超了上限）也不再续补：留着会让用户困在补参循环里
    # （下次输入继续往上叠，只会越补越错）。想重来就重发指令。
    #
    # ⚠️ 这里要带 session_key：补参凑齐后若命中多个候选，消歧还得靠它
    #    记状态 —— 否则「补参补齐」这条路径下 SD/DX 又会静默取首条。
    _PENDING.drop(session_key)
    return await handle_command(entry.names[0], collected, session_key=session_key)


async def reply_text(message_content: str, *, session_key: str | None = None) -> str:
    """把一条**非空**消息变成回复文本 —— 指令前缀的分发入口。
    ...
    if is_valid:
        ...
        return await handle_command(cmd_name, cmd_params, session_key=session_key)

    # 不是指令 —— 若这个会话正在等待，这条消息就是参数 / 选择
    resumed = await _resume_pending(session_key, message_content)
