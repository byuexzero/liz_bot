...
2. **定数一律是 float**。旧曲库把整数定数存成 int（``13`` 输出 ``"13"``），
   水鱼一律存 float（``13.0``）。这里用 :func:`_fmt_ds` **统一保留一位小数**
   （``13`` → ``"13.0"``）—— 定数是小数量，丢掉 ``.0`` 会与曲目的
   **等级**（``level``）混淆，详见 :func:`_fmt_ds` 的说明。
...
def _fmt_ds(value: Any) -> str:
    """定数转字符串，**固定保留一位小数**（``14.0`` → ``"14.0"``）。

    定数是一个小数量，整数值也必须带 ``.0``。压成 ``"14"`` 会丢掉"这是定数"
    这一信息，并与曲目的**等级**（``level``）长得一模一样 —— 两者并不等价：
    例如 id 112《Like the Wind [Reborn]》的 Expert 谱面等级是 ``11``、
    定数是 ``11.4``；Master 谱面等级 ``14``、定数 ``14.0``。
    发包相关流程若按显示值取定数，``14`` 会被误当作整数等级。

    .. note::

       这与重构前的观感**有意不同**。旧曲库把整数定数存成 ``int``，
       插值即得 ``"14"``；重构时曾为"逐字一致"而把 ``14.0`` 压回 ``14``，
       但那是在复刻一个缺陷，故改为始终保留一位小数。

    曲库实测：定数范围 ``1.0 ~ 15.0``，**无任何值超过一位小数**，
    因此 ``:.1f`` 不会截断信息。万一上游日后引入更多小数位，
    这里宁可原样输出也不静默四舍五入。
    """
